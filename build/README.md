...
This runbook sets up a single self-hosted PDS at `pds.alesr.me` with a `did:web`
identity — no plc.directory, no bsky.social.

## Prerequisites

- A server with a public IP (a $5 VPS or a Raspberry Pi with a public IP).
...
services:
  pocketpds:
    image: ghcr.io/alesr/pocketpds:latest   # or build from source
...
Caddy handles the TLS certificate. The PDS must be reachable over HTTPS at
`pds.alesr.me` because DID documents and handle resolution are served from it.
...
This returns a `did:web:pds.alesr.me` DID. The DID document is now served at
...
## 6. Relay (getting indexed)

For your posts to appear on bsky.app, a relay must crawl your PDS. The protocol
mechanism (`com.atproto.sync.requestCrawl`) is implemented, and PocketPDS
registers with relays that call `notifyOfUpdate`. The remaining step is
onboarding with a relay — bsky.network's policy on self-hosted PDSes has varied
over time, so check its current guidance. Alternative relays may accept
self-hosted PDSes directly.

## Caveats

- `did:web` has **no recovery key**. Your identity *is* the domain — keep the
  domain and the `POCKETPDS_SECRET` safe (the secret encrypts your account keys
  at rest).
